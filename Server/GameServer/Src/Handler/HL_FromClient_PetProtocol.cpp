...
//-------------------------------------------------------
ON_HANDLE(MGPT_CS_PET_STAR_UPGRADE_REQ, this_source_type, false)
{
	LC_ServerPlayer* pkPlayer = dynamic_cast<LC_ServerPlayer*>(pkTarget);
	if (NULL == pkPlayer)
		return;

	MG_CS_PetStarUpgrade msg;
	if (!msg.Decode(pMsgBody, usBodySize))
	{
		return;
	}

	pkPlayer->UpgradePetStarLevel(msg.m_nIndex);
	pkPlayer->SyncBackPackToClient(PACK_TYPE_BASIC);
}
ON_HANDLE(MGPT_CS_PET_DECOMPOSE_REQ, this_source_type, false)
{
	LC_ServerPlayer* pkPlayer = dynamic_cast<LC_ServerPlayer*>(pkTarget);
	if (NULL == pkPlayer)
		return;

	MG_CS_PetDecompose msg;
	if (!msg.Decode(pMsgBody, usBodySize))
	{
		return;
	}
	pkPlayer->DecomposePet(msg.m_nIndex);
}

ON_HANDLE(MGPT_CS_PET_ENABLE_REQ, this_source_type, false)
{
	LC_ServerPlayer* pkPlayer = dynamic_cast<LC_ServerPlayer*>(pkTarget);
	if (NULL == pkPlayer)
		return;

	MG_CS_PetOp_EnableReq msg;
	if (!msg.Decode(pMsgBody, usBodySize))
	{
		return;
	}
	pkPlayer->EnablePet(msg.m_nIndex);
}


ON_HANDLE(MGPT_CS_PET_DISABLE_REQ, this_source_type, false)
{
	LC_ServerPlayer* pkPlayer = dynamic_cast<LC_ServerPlayer*>(pkTarget);
	if (NULL == pkPlayer)
		return;

	MG_CS_PetOp_DisableReq msg;
	if (!msg.Decode(pMsgBody, usBodySize))
	{
		return;
	}
