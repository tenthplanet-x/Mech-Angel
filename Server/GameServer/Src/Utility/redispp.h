...
struct Command
{
	...
	std::string formate_cus_head(size_t numArgs);

#define RCMD_DECLARE_FUNC_ARG(z, n, arg) BOOST_PP_COMMA_IF(n) const T ## n& arg ## n
#define RCMD_WRITE_ARG(z, n, arg) dest->writeArg(arg ## n);
#define RCMD_DECLARE_FUNC_TEMP(n) template <BOOST_PP_ENUM_PARAMS(n, typename T), typename BufferType>
#define RCMD_DECLARE_FUNC_TEMP_EMPTY(n) template <typename BufferType>

#define RCMD_MAX_ARG_COUNT 20

#define RCMD_DECLARE_EXECUTE_FUNC(z, n, func) \
	BOOST_PP_IF(n, RCMD_DECLARE_FUNC_TEMP, RCMD_DECLARE_FUNC_TEMP_EMPTY)(n)\
	inline void func(BOOST_PP_REPEAT(n, RCMD_DECLARE_FUNC_ARG, arg) BOOST_PP_COMMA_IF(n) BufferType const& dest)\
	{\
		dest->write(header);\
		BOOST_PP_REPEAT(n, RCMD_WRITE_ARG, arg)\
	}\

	BOOST_PP_REPEAT(RCMD_MAX_ARG_COUNT, RCMD_DECLARE_EXECUTE_FUNC, execute);
...
class Connection
{
    ...
    friend class Transaction;
public:
    static const size_t kDefaultBufferSize = 512 * 1024;

    Connection(const std::string& host, const std::string& port, const std::string& password, size_t bufferSize = kDefaultBufferSize);
	...
	VoidReply ping(void);
	StringReply echo(const std::string& msg);
	...
	IntReply zadd(const std::string& key, int64_t score, const std::string& member);
	IntReply zrem(const std::string& key, const std::string& member);
	IntReply zcount(const std::string& key, int64_t _min, int64_t _max);
	IntReply zrank(const std::string& key, const std::string& member);
	IntReply zscore(const std::string& key, const std::string& member);
	IntReply zrevrank(const std::string& key, const std::string& member);
	StringReply zincrby(const std::string& key, int increment, const std::string& member);
	MultiBulkEnumerator zrange(const std::string& key, int start, int stop, const std::string& ws="WITHSCORES");
	MultiBulkEnumerator zrevrange(const std::string& key, int start, int stop, const std::string& ws="WITHSCORES");
	MultiBulkEnumerator zrangebyscore(const std::string& key, int64_t min, int64_t max, const std::string& ws="WITHSCORES");
	MultiBulkEnumerator zrevrangebyscore(const std::string& key, int64_t max, int64_t min, const std::string& ws="WITHSCORES");
	IntReply zremrangebyrank(const std::string& key, int start, int stop);
	...
	MultiBulkEnumerator hmget(const std::string& key, const std::vector<std::string>& fields);
	...
	MultiBulkEnumerator checkSub(void);
	...
	IntReply eval(const std::string& scripts, const std::vector<std::string>& keys, const std::vector<std::string>& args);
private:
	...
	void readCaseReply(boost::optional<std::string> &out);
    boost::optional<std::string> readBulkReply();
	boost::optional<std::string> readCaseReply();
	...
	DEFINE_COMMAND(Ping, 0);
	DEFINE_COMMAND(Echo, 1);
    ...
    DEFINE_COMMAND(ZRange, 4);
    DEFINE_COMMAND(ZRevRange, 4);
    DEFINE_COMMAND(ZRangeByScore, 4);
	DEFINE_COMMAND(ZRevRangeByScore, 4);
