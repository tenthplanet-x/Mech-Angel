...
HTTP_SESSION_HANDLE  HTTPClientOpenRequest (HTTP_CLIENT_SESSION_FLAGS Flags)
{
	...
	HTTPClientSetVerb((UINT64)pHTTPSession,(HTTP_VERB)HTTP_CLIENT_DEFAULT_VERB);    // Default HTTP verb
	...
	CHAR *pPassword)   // [IN] Null terminated string containing the sessions password
{
	...
	nLength = (UINT32)strlen(pUserName);
	...
	nLength = (UINT32)strlen(pPassword);
	...
	BOOL nInsert)           // [IN] Reserved could be any
{
	...
	nHeaderLength = (UINT32)strlen(pHeaderName);
	nDataLength   = (UINT32)strlen(pHeaderData);
	...
	UINT32 nClientPort)   //  [IN] Client side port 0 for none
{
	...
	do
	{
		...
		else
		{
			...
			if((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession,(CHAR*)"Host",4,pHTTPSession->HttpUrl.UrlHost.pParam,
				(UINT32)(pHTTPSession->HttpUrl.UrlPort.pParam - pHTTPSession->HttpUrl.UrlHost.pParam) -1 )) != HTTP_CLIENT_SUCCESS)
			{
				break;
			}
		}
		...
		if(pHTTPSession->HttpHeaders.HttpVerb == VerbPost && TotalLength == FALSE)
		{
			...
			if((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_SEND_CHUNKED) == HTTP_CLIENT_FLAG_SEND_CHUNKED)
			{
				...
				if((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession,(CHAR*)"Transfer-Encoding",17,(CHAR*)"chunked",7))!= HTTP_CLIENT_SUCCESS)
				{
					break;;
				}
			}
		...
		if((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession,(CHAR*)"User-Agent",10,(CHAR*)HTTP_CLIENT_DEFAULT_AGENT,(UINT32)strlen(HTTP_CLIENT_DEFAULT_AGENT)))!= HTTP_CLIENT_SUCCESS)
		{
			break;
		}
		...
		if((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_KEEP_ALIVE) == HTTP_CLIENT_FLAG_KEEP_ALIVE)
		{
			...
			{
				// No proxy keep alive:
				if((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession,(CHAR*)"Connection",10,(CHAR*)"Keep-Alive",10))!= HTTP_CLIENT_SUCCESS)
				{
					break;
				}
			}
			...
			{
				if((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession,(CHAR*)"Proxy-Connection",15,(CHAR*)"Keep-Alive",10))!= HTTP_CLIENT_SUCCESS)
				{
					break;
				}
			}
		...
		if(pHTTPSession->HttpHeaders.HttpVerb == VerbPost && TotalLength == TRUE)
		{
			...
			if((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession,(CHAR*)"Content-Length",14,ContentLength,(UINT32)strlen(ContentLength)))!= HTTP_CLIENT_SUCCESS)
			{
				break;
			}
		}
		...
		{
			if((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession,(CHAR*)"Cache-Control",13,(CHAR*)"no-cache",8))!= HTTP_CLIENT_SUCCESS)
			{
				break;
			}
		}
	...
	UINT32 nTimeout)
{
	...
	do
	{
		...
		if((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_SEND_CHUNKED) == HTTP_CLIENT_FLAG_SEND_CHUNKED)
		{
			...
			{
				nBytes = 2;;
				nRetCode = HTTPIntrnSend(pHTTPSession,(CHAR*)HTTP_CLIENT_CRLF,&nBytes);
				if(nRetCode != HTTP_CLIENT_SUCCESS)
				{
					break;
				}
...
UINT32 HTTPClientFindFirstHeader (HTTP_SESSION_HANDLE pSession, CHAR *pSearchClue,CHAR *pHeaderBuffer, UINT32 *nLength)
{
	...
	nClueLength = (UINT32)strlen(pSearchClue); // See if we are not to long
...
UINT32 HTTPClientGetNextHeader (HTTP_SESSION_HANDLE pSession, CHAR *pHeaderBuffer, UINT32 *nLength)
{
	...
	{
		nOffset = (UINT32)(pHTTPSession->HttpHeaders.HeaderSearch.pParam - pHTTPSession->HttpHeaders.HeadersIn.pParam);
	}
	...
	UINT32 nUrlLength)  // [IN] The length the Url string
{
	...
	if(HTTPStrSearch(pUrlPtr->Url,(CHAR*)":",nUrlOffset,nUrlLength,&pUrlPtr->UrlBsee) == FALSE)
	{
	...
	if(HTTPStrInsensitiveCompare(pUrlPtr->UrlBsee.pParam,(CHAR*)"https",pUrlPtr->UrlBsee.nLength) == TRUE)
	{
	...
	{
		if(HTTPStrInsensitiveCompare(pUrlPtr->UrlBsee.pParam,(CHAR*)"http",pUrlPtr->UrlBsee.nLength) == FALSE)
		{
	...
	if(HTTPStrSearch(pUrlPtr->Url,(CHAR*)"://",nUrlOffset,3,0) == FALSE)
	{
	...
	// Get the host name
	if(HTTPStrSearch(pUrlPtr->Url,(CHAR*)"/",nUrlOffset,(nUrlLength - nUrlOffset),&pUrlPtr->UrlHost) == FALSE)
	{
	...
	if(HTTPStrSearch(pUrlPtr->Url,(CHAR*)":",
		(nUrlOffset - pUrlPtr->UrlHost.nLength),
...

{
	...
	do
	{
		...
		nBytes = (UINT32)(HttpParam.pParam - pHTTPSession->HttpHeaders.HeadersOut.pParam);
...
UINT32 HTTPIntrnConnectionOpen (P_HTTP_SESSION pHTTPSession)
{
	...
	do
	{
		...
		if(pHTTPSession->HttpConnection.HttpSocket == HTTP_INVALID_SOCKET)
		{
			...
			pHTTPSession->HttpConnection.HttpSocket = (INT32)socket( AF_INET,	     // Address family 
...
#ifdef _WIN32
		nRetCode = ioctlsocket(pHTTPSession->HttpConnection.HttpSocket, FIONBIO, (u_long*)&nNonBlocking);
#else
		...
		*/
		...
		{ // Is it a TLS connection?
			nRetCode = HTTPWrapperSSLConnect(pHTTPSession->HttpConnection.HttpSocket,	// Socket
				...
				(CHAR*)"desktop");	                            // Hostname (ToDo: Fix this)	                    
		}
		else    // Non TLS so..
		{
		...
		while(1)
		{
			...
			if(FD_ISSET(pConnection->HttpSocket ,&pConnection->FDWrite))
			{
				...
				if((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_SECURE) == HTTP_CLIENT_FLAG_SECURE)
				{
					...
					{
						nRetCode = HTTPWrapperSSLNegotiate(pConnection->HttpSocket,0,0,(CHAR*)"desktop");
						if(nRetCode != 0)
						{
							// TLS Error
	...
	UINT32 nOffset)         //  [IN]  Optionaly privide an offset to start looking from
{
	...
	do
	{
		...
		nLength = (UINT32)strlen(pHeaderName);
		...
		if(pParam->pParam) // Did we find it?
		{
			...
			if(pHeaderEnd)
			{
				...
				pParam->nLength =  (UINT32)(pHeaderEnd -  pParam->pParam);   
				nRetCode = HTTP_CLIENT_SUCCESS;
				break;
...
UINT32 HTTPIntrnHeadersParse (P_HTTP_SESSION pHTTPSession)
{
	...
	do
	{
		...
		if(HTTPStrInsensitiveCompare(pPtr,(CHAR*)"http",4) != TRUE)
		{
		...
		// Look for the token
		if(HTTPIntrnHeadersFind(pHTTPSession,(CHAR*)"content-length",&HTTPParam,TRUE,0) == HTTP_CLIENT_SUCCESS)
		{
		...
		if( (HTTPIntrnHeadersFind(pHTTPSession,(CHAR*)"connection",&HTTPParam,TRUE,0) == HTTP_CLIENT_SUCCESS) ||
			(HTTPIntrnHeadersFind(pHTTPSession,(CHAR*)"proxy-connection",&HTTPParam,TRUE,0) == HTTP_CLIENT_SUCCESS))
		{
			...
			if(HTTPStrGetToken(HTTPParam.pParam,HTTPParam.nLength,HTTPToken,&nTokenLength))
			{
				...
				pHTTPSession->HttpHeadersInfo.Connection = HTTPStrInsensitiveCompare(HTTPToken,(CHAR*)"keep-alive",0);
				// Is it a closed session
				if(HTTPStrInsensitiveCompare(HTTPToken,(CHAR*)"close",0) == TRUE)
				{
		...
		if(HTTPIntrnHeadersFind(pHTTPSession,(CHAR*)"transfer-encoding",&HTTPParam,TRUE,0) == HTTP_CLIENT_SUCCESS)
		{
			...
			if(HTTPStrGetToken(HTTPParam.pParam,HTTPParam.nLength,HTTPToken,&nTokenLength))
			{
				...
				if(HTTPStrInsensitiveCompare(HTTPToken,(CHAR*)"chunked",0) == TRUE)
				{
		...
		while(AuthHeaders == FALSE)  // address multiple authentication methods presented by the server
		{
			if(pHTTPSession->HttpHeadersInfo.nHTTPStatus == HTTP_STATUS_UNAUTHORIZED)
			{
				...
				if(HTTPIntrnHeadersFind(pHTTPSession,(CHAR*)"www-authenticate",&pHTTPSession->HttpAuthHeader.AuthHeader,TRUE,nOffset) != HTTP_CLIENT_SUCCESS)
				{
				...
				else
				{
					...
					nOffset = (UINT32)(pHTTPSession->HttpAuthHeader.AuthHeader.pParam - pHTTPSession->HttpHeaders.HeadersIn.pParam) + 3;
				}
			}
			else
			{
		...
		if(pHTTPSession->HttpHeadersInfo.nHTTPStatus == HTTP_STATUS_PROXY_AUTHENTICATION_REQUIRED)
		{
			...
			if (HTTPIntrnHeadersFind(pHTTPSession,(CHAR*)"proxy-authenticate",&pHTTPSession->HttpAuthHeader.AuthHeader,TRUE,0) != HTTP_CLIENT_SUCCESS)
			{
				nRetCode = HTTP_CLIENT_ERROR_BAD_HEADER;
				break;
			}
			...
			(pHTTPSession->HttpHeadersInfo.nHTTPStatus == HTTP_STATUS_OBJECT_MOVED_PERMANENTLY))
		{
			...
			if (HTTPIntrnHeadersFind(pHTTPSession,(CHAR*)"location",&pHTTPSession->HttpHeadersInfo.HttpRedirectURL,TRUE,0) != HTTP_CLIENT_SUCCESS)
			{
	...
	// Can be used for evaluating a HEAD request
{
	...
	do
	{
		...
		if(pHTTPSession->HttpHeaders.HttpVerb == VerbHead)
		{
			...
			{
				// Chunking
				if((nRetCode = HTTPIntrnHeadersRemove(pHTTPSession,(CHAR*)"Transfer-Encoding")) != HTTP_CLIENT_SUCCESS)
				{
					break;
				}
			}
			...
			{
				if((nRetCode = HTTPIntrnHeadersRemove(pHTTPSession,(CHAR*)"Content-Length")) != HTTP_CLIENT_SUCCESS)
				{
					break;
				}
			}
		...
		// Request Verb
		nBytes = (UINT32)strlen(pHTTPSession->HttpHeaders.Verb) + 1;
		...
		{
			nBytes = (UINT32)strlen(pHTTPSession->HttpUrl.Url);
		...
		nBytes = (UINT32)strlen(RequestCmd);
		if((nRetCode = HTTPIntrnSend(pHTTPSession,RequestCmd,&nBytes)) != HTTP_CLIENT_SUCCESS)  
		{
			break;
		}
		...
		nBytes = (UINT32)strlen(HTTP_CLIENT_CRLF);
		if((nRetCode = HTTPIntrnSend(pHTTPSession,(CHAR*)HTTP_CLIENT_CRLF,&nBytes)) != HTTP_CLIENT_SUCCESS)
		{
			break;
		}
		// Set the counters
...
UINT32 HTTPIntrnAuthSendBasic (P_HTTP_SESSION pHTTPSession)
{
	...
	CHAR*       pPtr;
	CHAR*       INITIAL_HDR         = (CHAR*)"Authorization: Basic ";
	CHAR*       INITIAL_PROXY_HDR   = (CHAR*)"Proxy-Authorization: Basic ";
	...
	do
	{
		...
		case( HTTP_STATUS_UNAUTHORIZED): // For host authentication
			{
				...
				nSrcLength  = (UINT32)strlen(Cred);
				nDestLength = HTTP_CLIENT_MAX_64_ENCODED_CRED;
				nSegmentLength = (UINT32)strlen(INITIAL_HDR); 
				...
				nDestLength = (UINT32)strlen(Cred64);
		...
		case (HTTP_STATUS_PROXY_AUTHENTICATION_REQUIRED): // For Proxy authentication
			{
				...
				nDestLength = (UINT32)strlen(Cred64);
				nSegmentLength = (UINT32)strlen(INITIAL_PROXY_HDR);
				// Build and send the data first the hard-coded static portion
		...
		// Terminating CRLF
		nSegmentLength = (UINT32)strlen((CHAR*)HTTP_CLIENT_CRLF);
...
UINT32 HTTPIntrnAuthSendDigest (P_HTTP_SESSION pHTTPSession)
{
	...
	CHAR*       INITIAL_HDR         = (CHAR*)"Authorization: Digest username=\"";
	CHAR*       INITIAL_PROXY_HDR   = (CHAR*)"Proxy-Authorization: Digest username=\"";
	CHAR*       REALEM_HDR          = (CHAR*)"\", realm=\"";
	CHAR*       QOP_HDR             = (CHAR*)"\", qop=\"";
	CHAR*       ALGO_HDR            = (CHAR*)"\", algorithm=\"";
	CHAR*       URI_HDR             = (CHAR*)"\", uri=\"";
	CHAR*       NONCE_HDR           = (CHAR*)"\", nonce=\"";
	CHAR*       NC_HDR              = (CHAR*)"\", nc=00000001, cnonce=\""; // To-Do: This should be tested!!
	CHAR*       RSP_HDR             = (CHAR*)"\", response=\"";
	CHAR*       OPQ_HDR             = (CHAR*)"\", opaque=\"";
	...
	do
	{
		...
		nSegmentLength = (UINT32)strlen(pHTTPSession->HttpCredentials.CredUser);
		...
		nSegmentLength = (UINT32)strlen(REALEM_HDR);
		...
		if((nRetCode = HTTPStrGetDigestToken(pHTTPSession->HttpAuthHeader.AuthHeader,(CHAR*)"realm", &HttpParamRealm)) != HTTP_CLIENT_SUCCESS)
		{
			break;
		}
		...
		nSegmentLength = (UINT32)strlen(QOP_HDR);
		...
		if((nRetCode = HTTPStrGetDigestToken(pHTTPSession->HttpAuthHeader.AuthHeader,(CHAR*)"qop", &HttpParamQop)) != HTTP_CLIENT_SUCCESS)
		{
			break;
		}
		...
		nSegmentLength = (UINT32)strlen(ALGO_HDR);
		...
		if((nRetCode = HTTPStrGetDigestToken(pHTTPSession->HttpAuthHeader.AuthHeader,(CHAR*)"algorithm", &HttpParamAlg)) != HTTP_CLIENT_SUCCESS)
		{

			// The server did not state its required algorithm so use the default
			HttpParamAlg.pParam  =   (CHAR*)HTTP_CLIENT_DEFAULT_DIGEST_AUTH;
			HttpParamAlg.nLength =   (UINT32)strlen((CHAR*)HTTP_CLIENT_DEFAULT_DIGEST_AUTH);
		}
		// Get the algorithem type 
		if(HTTPStrInsensitiveCompare(HttpParamAlg.pParam ,(CHAR*)"md5",3 ) == TRUE)
		{
			if(HTTPStrInsensitiveCompare(HttpParamAlg.pParam ,(CHAR*)"md5-sess", HttpParamAlg.nLength) == TRUE)
			{
				nAlgType = 1;
			}
		...
		// "Authorization: Digest username="username", realm="myRealm", qop="auth",
		// algorithm="MD5", uri="
		nSegmentLength = (UINT32)strlen(URI_HDR);
		...
		nSegmentLength = (UINT32)strlen(pHTTPSession->HttpUrl.UrlRequest.pParam);
		...
		nSegmentLength = (UINT32)strlen(NONCE_HDR);
		...
		if((nRetCode = HTTPStrGetDigestToken(pHTTPSession->HttpAuthHeader.AuthHeader,(CHAR*)"nonce", &HttpParamNonce)) != HTTP_CLIENT_SUCCESS)
		{
			break;
		}
		...
		nSegmentLength = (UINT32)strlen(NC_HDR);
		...
		nSegmentLength = (UINT32)strlen(Cnonce);
		...
		if((nRetCode = HTTPStrGetDigestToken(pHTTPSession->HttpAuthHeader.AuthHeader,(CHAR*)"opaque", &HttpParamOpq)) == HTTP_CLIENT_SUCCESS)
		{

			nSegmentLength = (UINT32)strlen(OPQ_HDR);
		...
		nSegmentLength = (UINT32)strlen(RSP_HDR);
		...
		nSegmentLength = (UINT32)strlen(Response);
		...
		// Terminate 0x24 (")
		nSegmentLength = 1;
		if((nRetCode = HTTPIntrnSend(pHTTPSession,(CHAR*)"\"", &nSegmentLength)) != HTTP_CLIENT_SUCCESS) 
		{
			break;
		}
		// Set the counters
		...
		// Terminating CRLF
		nSegmentLength = (UINT32)strlen(HTTP_CLIENT_CRLF);
		if((nRetCode = HTTPIntrnSend(pHTTPSession,(CHAR*)HTTP_CLIENT_CRLF, &nSegmentLength)) != HTTP_CLIENT_SUCCESS) 
		{
			break;
		}
		// Set the counters
