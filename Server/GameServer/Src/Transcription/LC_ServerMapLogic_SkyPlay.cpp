...
LC_ServerMapLogic_SkyPlay::LC_ServerMapLogic_SkyPlay():
m_Config(NULL),m_State(Sky_State_None),m_nEntryPlayerNum(0),
...
bool LC_ServerMapLogic_SkyPlay::Init(LC_ServerMapLogic_Place* place,LC_Dule_Config* config)
{
	if (place==NULL || config==NULL)
		return false;
	if ( false == LC_ServerMapLogic_PlayBase::Init(place,PlayType_Sky))
		return false;
	m_nEntryPlayerNum = 0;
	m_Config=config;
	...
	m_vecResult.clear();
	m_State=Sky_State_None;
	m_mapID  = SKY_SOLO_DULE_RES_ID;
	return true;
}

void LC_ServerMapLogic_SkyPlay::UnInit()
{
	End();
	// relase
...
bool LC_ServerMapLogic_SkyPlay::Start()
{
	if (m_State!=Sky_State_Inited && m_State!=Sky_State_End)
	{
		return false;
	}
	SetSkyNextState(Sky_State_Prepare);
	return true;
}
void LC_ServerMapLogic_SkyPlay::ClearBuddyOp(LC_ServerPlayer* pPlayer)
{
...
void LC_ServerMapLogic_SkyPlay::SyncFightReportToGroup()
{

}

void LC_ServerMapLogic_SkyPlay::updateNullTeamIDFailed()
{
...
bool LC_ServerMapLogic_SkyPlay::CheckCanNextTeamBattle()
{
	for (int i = 0; i < m_vecResult.size(); ++i )
	{
		if( m_vecResult[i].uIndex > m_uCurrIndex )
...
void LC_ServerMapLogic_SkyPlay::updateRemainTeamFailed(int currIndex,bool bChallengeFailed)
{
	for (int i = 0; i < m_vecResult.size(); ++i )
	{
...
bool LC_ServerMapLogic_SkyPlay::PrepareNextTeamBattle()
{
	...
	if(false == CheckCanNextTeamBattle())
	{
		...
		SetSkyNextState(Sky_State_BattleEnd);
		return false;
	}
	m_uCurrIndex++;
...
void LC_ServerMapLogic_SkyPlay::End()
{
	if (!CheckRun())
		return;
	// 停止战斗行为
	...
	SetSkyNextState(Sky_State_End);
}
void LC_ServerMapLogic_SkyPlay::SetSkyNextState(Sky_State eState)
{
	GET_PROCESS_TICK;
	if( m_State >= Sky_State_WaitReward && eState <= Sky_State_WaitReward)
		return;
	

	if(eState ==Sky_State_LoadPlayerInfo)
	{
		m_TimeStamp=GET_PROCESS_TIME;
	}
	else if(eState ==Sky_State_ShowAllInfo)
	{
		int ShowAllInfoTime = m_Config->ShowAllInfoTime;
		m_TimeStamp=GET_PROCESS_TIME+ShowAllInfoTime;
	}
	else if(eState == Sky_State_PrepareNextTeam)
	{
		m_TimeStamp=GET_PROCESS_TIME+1;
	}
	else if(eState == Sky_State_ShowTeamInfo)
	{
		int ShowTeamInfoTime = m_Config->ShowTeamInfoTime;
		m_TimeStamp=GET_PROCESS_TIME+ShowTeamInfoTime;
	}
	else if(eState == Sky_State_Battle)
	{
		int BattleTime = m_Config->BattleTime;
		m_TimeStamp=GET_PROCESS_TIME+BattleTime;
	}
	else if(eState == Sky_State_BattleEnd)
	{
		m_TimeStamp=GET_PROCESS_TIME;
	}
	else if(eState == Sky_State_WaitReward)
	{
		m_TimeStamp= GET_PROCESS_TIME+10 ;
	}
	else if(eState == Sky_State_End)
	{
		m_TimeStamp=GET_PROCESS_TIME;
	}
	m_State =  eState;
}
bool LC_ServerMapLogic_SkyPlay::Update(float fCurrentTime, float fDeltaTime)
{
	if(m_TimeStamp == 0.0f || m_TimeStamp > fCurrentTime || m_State >= Sky_State_End)
		return true;
	if(m_State ==Sky_State_LoadPlayerInfo)
	{
		// SummonPlayerFinish
		if( m_nEntryPlayerNum == 2 )
		{
			InitAllBuddyInfo();
			//
			SetSkyNextState(Sky_State_ShowAllInfo);
			// 展示全部信息
			NotifyShowAllInfoTime();
			m_nEntryPlayerNum = 0;
		}
	}
	else if(m_State == Sky_State_ShowAllInfo)
	{
		NotifyShowTeamInfoTime();
		SetSkyNextState(Sky_State_ShowTeamInfo);
	}
	else if(m_State ==Sky_State_PrepareNextTeam)
	{
		// 
	...
	else if(m_State == Sky_State_ShowTeamInfo)
	{
		Battle();
		SetSkyNextState(Sky_State_Battle);
	}
	else if(m_State == Sky_State_Battle)
	{
		// 一轮战斗结束 时间到
		BattleEnd(0,NULL,true);
	}
	else if(m_State == Sky_State_BattleEnd)
	{
		// 判定 本场 胜负
		Settle();
		// 发奖 
	...
	else if(m_State == Sky_State_WaitReward )
	{
...
ResultType LC_ServerMapLogic_SkyPlay::CheckEnter(LC_ServerPlayer* player)
{
...
Utility::UT_Vec3Int	LC_ServerMapLogic_SkyPlay::ComputeEnterLocation(LC_ServerPlayer* player,const Utility::UT_Vec3Int& targetlocation)
{
	return m_Config->Factions[0].Position;
}

void LC_ServerMapLogic_SkyPlay::EventNotify(MapLogicEvent& kEvent)
{
	
}

void LC_ServerMapLogic_SkyPlay::PlayerEventNotify(LC_ServerPlayer* player, MapLogicEvent& kEvent)
{
	IF_A_NA_PLAYER(player)
		return ;
	if ( kEvent.Type() == PlayerEvent_Enter)
	{
		...
		Prepare();
		m_nEntryPlayerNum++;
		SetSkyNextState(Sky_State_LoadPlayerInfo);
...
void LC_ServerMapLogic_SkyPlay::SetRoundBattleResult(LC_ServerPlayer* player,bool timeOut)
{
...
bool LC_ServerMapLogic_SkyPlay::CheckOwnerDead( LC_ServerPlayer* pPlayer, object_id_type& nDeadPlayerID )
{
	bool bResult = false;
...
bool LC_ServerMapLogic_SkyPlay::CheckEnd()
{
	return m_State == Sky_State_End;
}

bool LC_ServerMapLogic_SkyPlay::CheckRun()
{
	if (	m_State == Sky_State_Prepare 
		||	m_State == Sky_State_LoadPlayerInfo
		||	m_State == Sky_State_ShowAllInfo 
		||	m_State == Sky_State_ShowTeamInfo 
		||	m_State == Sky_State_PrepareNextTeam
		||	m_State == Sky_State_Battle
		||	m_State == Sky_State_BattleEnd
		||	m_State == Sky_State_WaitReward)
	{
		return true;
	}
	return false;
}
void LC_ServerMapLogic_SkyPlay::Settle(bool bLeave)
{
	// 判定结果
	if(bLeave)
	{
...
void LC_ServerMapLogic_SkyPlay::SendReward()
{
	SetSkyNextState(Sky_State_WaitReward);
	//结算
	if(false == CheckChallengeBeChallengePtr())
		return;
...
void LC_ServerMapLogic_SkyPlay::ChallengePlayerActionForbid(LC_ServerPlayer* pPlayer)
{
...
void LC_ServerMapLogic_SkyPlay::ChallengePlayerActionUnForbid(LC_ServerPlayer* pPlayer)
{
	LC_ActionForbid* pa = pPlayer->GetActionForbid();
...
void LC_ServerMapLogic_SkyPlay::ReservaDuleInfo(object_id_type challengeid,const unique_id_impl& bechallengeid)
{
...
void LC_ServerMapLogic_SkyPlay::Prepare()
{
	LC_ServerPlayer *bechallengeplayer=GetPlace()->SpawnPlayer(m_BeChallengeCharID, m_Config->Factions[1].Position, PLY_DEF_CLONE_BITS, NULL, LC_ServerMapLogic_SkyPlay::SummonPlayerFinish, FACTION_NEUTRAL);
...
void LC_ServerMapLogic_SkyPlay::InitAllBuddyInfo()
{
	if(false == CheckChallengeBeChallengePtr())
		return;
...
void LC_ServerMapLogic_SkyPlay::PackSkyRoundTeamDeadInfo(MG_NOTIFY_SkyRoundShowInfo& notifyMsg)
{
	notifyMsg.m_LeftDeadTeamIdVec.clear();
	notifyMsg.m_RightDeadTeamIdVec.clear();
	for (int i = 0; i <m_vecResult.size(); ++i)
	{
		SingleSkyTeamFightInfo &teamInfo = m_vecResult[i];
...
void LC_ServerMapLogic_SkyPlay::UpdateBuddyDeadInfo(LC_ServerPlayer* pPlayer,mem::vector <int>& vecReliveBuddyID )
{
...
void LC_ServerMapLogic_SkyPlay::UpdateTeamDeadInfo(int nCurrIndex,unique_id_impl uWindPlayerId,bool bChallenge )
{
	for (int i = 0; i <m_vecResult.size(); ++i)
	{
		SingleSkyTeamFightInfo &teamInfo = m_vecResult[i];
...
void LC_ServerMapLogic_SkyPlay::InitTeamFightId(int nCurrIndex)
{
	for (int i = 0; i <m_vecResult.size(); ++i)
	{
		SingleSkyTeamFightInfo &teamInfo = m_vecResult[i];
...
void LC_ServerMapLogic_SkyPlay::NotifyShowAllInfoTime()
{
	if(false == CheckChallengeBeChallengePtr())
		return;
	int ShowAllInfoTime = m_Config->ShowAllInfoTime;
...
void LC_ServerMapLogic_SkyPlay::NotifyShowTeamInfoTime()
{
	if(false == CheckChallengeBeChallengePtr())
		return;
	int ShowTeamInfoTime = m_Config->ShowTeamInfoTime;
...
void LC_ServerMapLogic_SkyPlay::PlayerLevave(int opType)
{
...
void LC_ServerMapLogic_SkyPlay::BattleEnd(int endType,LC_ServerPlayer* pPlayer,bool timeOut)
{
	if(false == CheckChallengeBeChallengePtr())
		return;
	// 停止 双方的打斗
	...
	if(timeOut)
	{
		...
		SetSkyNextState(Sky_State_PrepareNextTeam);
		return;
	}
	if (endType==PlayerEvent_Leave)
	{
		if(m_State == Sky_State_PrepareNextTeam || m_State == Sky_State_BattleEnd)
			return;
...
void LC_ServerMapLogic_SkyPlay::Battle()
{
	if(false == CheckChallengeBeChallengePtr())
		return;
...
void LC_ServerMapLogic_SkyPlay::InitHatred( LC_ServerPlayer* pPlayerAController, LC_ServerPlayer* pPlayerBController )
{
...
void LC_ServerMapLogic_SkyPlay::InitBattle( LC_ServerPlayer* pPlayerAController, LC_ServerPlayer* pPlayerBController )
{
...
bool LC_ServerMapLogic_SkyPlay::CheckChallengeBeChallengePtr()
{
	LC_ServerPlayer* challengePlayer=GetPlace()->GetActorManger().GetPlayer(m_ChallengePlayerID);
...
void LC_ServerMapLogic_SkyPlay::SendErrorReward()
{
	LC_ServerPlayer* challengePlayer=GetPlace()->GetActorManger().GetPlayer(m_ChallengePlayerID);
	IF_A_NA_PLAYER(challengePlayer)
		return ;
...
void LC_ServerMapLogic_SkyPlay::UpLoadSoloDulePlayInfoToGroup(LC_ServerPlayer* challengePlayer,LC_ServerPlayer* bechallengePlayer)
{
	...
	{
		SingleSkyTeamFightInfo & singleInfo = m_vecResult[i];
	...
	reqMsg.m_solo_result.m_nAreanType = Arena_Type_Firmament;// 结果 0：Arena_Type_Challenge  1：Arena_Type_Firmament 2:Arena_Type_Peak
	...
	if(m_bChallengeWind)
	{
		...
		kEvent.SetNum(1);
		challengePlayer->UpdateAchievementRecord(LC_ACHIEVEMENT_TYPE_SKY_SOLODULE_WIN, kEvent);
	}
}

void LC_ServerMapLogic_SkyPlay::SummonPlayerFinish(LC_ServerPlayer* player)
{
	...
	if (place==NULL)
		return;
	LC_ServerMapLogic_SkyPlay* play=(LC_ServerMapLogic_SkyPlay*)place->GetPlay(PlayType_Sky);
	if (play==NULL)
		return;
	char_data_define::save_skill_state_info info;
	...
	play->SetSkyNextState(Sky_State_LoadPlayerInfo);
...
bool LC_ServerMapLogic_SkyPlay::InitTeamInfo(bool challengeFlag)
{
	if(challengeFlag)
	{
		LC_ServerPlayer* challengePlayer=GetPlace()->GetActorManger().GetPlayer(m_ChallengePlayerID);
		...
		{
			SingleSkyTeamFightInfo singleInfo;
	...
	else
	{
		...
		if(NULL == pDuelinfo)
			return false;
		m_BeChallengePlayer->SetSkyArenaScoreValue(pDuelinfo->m_uScore);	
		...
		if(vecBeChallengeResult.size() >= m_vecResult.size() )
		{
			...
			{
				//int64 nFightId =GameLogic::LC_Helper::generate_unique_id(0, GetGameServerID());
				if( i < m_vecResult.size() )
				{
					m_vecResult[i].uBeChallengeTeamId = vecBeChallengeResult[i];
					//m_vecResult[i].nFightId = nFightId;
				}
				else
				{
					SingleSkyTeamFightInfo singleInfo;
